...
```
{
  ...
  "metadata": {
    // A blob of keys useful for key rotation. 
    // Each metadata key is encrypted to all public keys that have access to the share. 
    // It is generated by client on first upload.
    // If a recipient has been removed of a share a new metadata key will be generated 
    // and the client always uses the newest one.
    // Encrypted files refer to which metadata key to use. 
    // In case of updating a file the client should update it with the new metadata encryption key.
